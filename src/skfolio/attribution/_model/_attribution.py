...
class Attribution:
    ...
    def __post_init__(self):
        ...
        # A rolling attribution is a stack of single-point ones: every field carries
        # one extra leading window axis and is otherwise checked identically, so the
        # expected shape prefix is what drives the whole validation.
        leading = (len(self.observations),) if self.is_rolling else ()
        ...
        for name, component in components:
            _validate_component(name, component, leading)
        ...
        for name, breakdown in [
            ...
            if breakdown is not None:
                _validate_breakdown(name, breakdown, leading)

        self._validate_asset_by_factor_contrib(leading)

    def _validate_asset_by_factor_contrib(self, leading: tuple[int, ...]) -> None:
        """Validate `asset_by_factor_contrib` shapes and names.

        The names are checked against the `assets` and `factors` breakdowns.

        Parameters
        ----------
        leading : tuple[int, ...]
            The leading axes every field must carry: `(n_windows,)` for a rolling
            attribution, empty for a single-point one.

        Raises
        ------
        ValueError
            If a field does not have the expected number of axes or shape, or if the
            asset or factor names disagree with the corresponding breakdown.
        """
        if self.asset_by_factor_contrib is None:
            return

        afc = self.asset_by_factor_contrib
        expected = (*leading, len(afc.asset_names), len(afc.factor_names))
        hint = (
            "(n_windows, n_assets, n_factors)" if leading else "(n_assets, n_factors)"
        )
        for field in ("vol_contrib", "mu_contrib"):
            value = getattr(afc, field)
            label = f"asset_factor_contribs.{field}"
            _check_ndim(label, value, expected, hint)
            _check_shape(label, value, expected)

        if self.assets is not None and not np.array_equal(
            afc.asset_names, self.assets.names
        ):
            raise ValueError(
                "`asset_factor_contribs.asset_names` does not match `assets.names`."
            )
        if self.factors is not None and not np.array_equal(
            afc.factor_names, self.factors.names
        ):
            raise ValueError(
    ...
    def plot_vol_contrib(
        ...
        return _plot_contribution_chart(
            data=data,
            idio=self.idio,
            top_n=top_n,
    ...
    def plot_exposure(
        ...
        """
        data = self._get_breakdown_data(by_family)
        names, exposure, exposure_std, colors = _prepare_exposure_plot_data(
            data=data,
            top_n=top_n,
            is_rolling=self.is_rolling,
        )
        ...
        has_exposure_std = (
            show_std and exposure_std is not None and np.any(np.isfinite(exposure_std))
        )
        component_label = "Family" if by_family else "Factor"
        ...
        if self.is_rolling:
            ...
            band_width = None
            if has_exposure_std:
                customdata = [exposure_std[:, i] for i in range(len(names))]
                band_width = exposure_std
                hovertemplates = [
                    f"<b>{_html_escape(name)}</b><br>"
                    "Observation=%{x}<br>"
                    "Exposure=%{y:.4f}<br>"
                    "Exposure Std=%{customdata:.4f}"
                    "<extra></extra>"
                    for name in names
                ]
                layout_title: str | dict[str, Any] = {
                    "text": (
                        f"{title} Over Time<br><sup style='font-size:0.7em'>"
                        "Band: +/- 1 exposure std</sup>"
                    )
                }
            else:
                hovertemplates = [
            ...
            fig = _plot_rolling_lines(
                ...
                legend_title_text=component_label,
                hovertemplates=hovertemplates,
                customdata=customdata,
                band_width=band_width,
                band_label="Exposure Std",
            )
        else:
            fig = go.Figure()
            ...
            if has_exposure_std:
                bar_kwargs["customdata"] = exposure_std[0]
                bar_kwargs["error_y"] = dict(
                    type="data",
                    array=exposure_std[0],
                    visible=True,
                ...
                bar_kwargs["hovertemplate"] = (
                    "<b>%{x}</b><br>Exposure=%{y:.4f}<extra></extra>"
                )
            layout_title: str | dict[str, Any] = title
            if has_exposure_std:
                layout_title = {
                    "text": (
                        f"{title}<br><sup style='font-size:0.7em'>"
                        "Error bars: +/- 1 exposure std</sup>"
                    )
                }
    ...
    def plot_return_vs_vol_contrib(
        ...
        fig.update_layout(
            title=title,
            xaxis_title="Annualized Volatility Contribution (%)",
...
# Breakdown fields common to factors, families and assets, holding one value per item.
_BREAKDOWN_FIELDS = (
    "exposure",
    "vol",
    "vol_contrib",
...
# Optional breakdown fields, with whether a scalar is an accepted value: `weight_std`
# is 0.0 for a static allocation, the others are arrays whenever they are set.
_OPTIONAL_BREAKDOWN_FIELDS = (
    ("exposure_std", False),
    ("weight_std", True),
    ("mu_contrib_uncertainty", False),
)


def _check_ndim(
    label: str, value: AnyArray, expected: tuple[int, ...], hint: str
) -> None:
    """Raise if `value` does not have as many axes as `expected`.

    Parameters
    ----------
    label : str
        The field path reported in the error, e.g. `"factors.exposure"`.

    value : ndarray
        The value to check.

    expected : tuple[int, ...]
        The expected shape, whose length is the expected number of axes.

    hint : str
        The names of the expected axes, e.g. `"(n_windows, n_items)"`.

    Raises
    ------
    ValueError
        If `value.ndim` differs from `len(expected)`.
    """
    if value.ndim != len(expected):
        mode = "Rolling" if len(expected) > 1 else "Single-point"
        raise ValueError(
            f"{mode} attribution requires `{label}` to be {len(expected)}D {hint}, "
            f"got {value.ndim}D."
        )


def _check_shape(
    label: str, value: AnyArray, expected: tuple[int, ...], suffix: str = ""
) -> None:
    ...
    suffix : str, default=""
        Appended to the error, naming what the shape should have matched.

    Raises
    ------
    ValueError
        If `value.shape` differs from `expected`.
    """
    if value.shape != expected:
        raise ValueError(
            f"`{label}` has shape {value.shape}, expected {expected}{suffix}."
        )


def _validate_component(
    name: str, component: Component, leading: tuple[int, ...]
) -> None:
    """Validate a `Component`'s fields.

    Parameters
    ----------
    name : str
        The component name reported in the error, e.g. `"systematic"`.

    component : Component
        The component to validate.

    leading : tuple[int, ...]
        `(n_windows,)` for a rolling attribution, empty for a single-point one, where
        the fields are scalars.

    Raises
    ------
    ValueError
        If a field is not a scalar (single-point) or a 1D array of `n_windows`
        elements (rolling).
    """
    if not leading:
        for field in _COMPONENT_FIELDS:
            ...
            if isinstance(value, np.ndarray):
                raise ValueError(
                    f"Single-point attribution requires `{name}.{field}` "
                    f"to be a scalar, got array with shape {value.shape}."
                )
        return

    fields_to_check = list(_COMPONENT_FIELDS)
    if component.mu_uncertainty is not None:
        fields_to_check.append("mu_uncertainty")

    for field in fields_to_check:
        value = getattr(component, field)
        if not isinstance(value, np.ndarray) or value.ndim != 1:
            ndim = f" with ndim={value.ndim}" if isinstance(value, np.ndarray) else ""
            raise ValueError(
                f"Rolling attribution requires `{name}.{field}` to be "
                f"a 1D array, got {type(value).__name__}{ndim}."
            )
        _check_shape(
            f"{name}.{field}", value, leading, suffix=" to match `observations`"
        )
...
def _validate_breakdown(
    ...
    """Validate a breakdown's fields against its number of items.

    Parameters
    ----------
    name : str
        The breakdown name reported in the error, one of `"factors"`, `"families"` or
        `"assets"`.

    breakdown : FactorBreakdown | FamilyBreakdown | AssetBreakdown
        The breakdown to validate.

    leading : tuple[int, ...]
        `(n_windows,)` for a rolling attribution, empty for a single-point one.

    Raises
    ------
    ValueError
        If a field does not have the expected number of axes or shape.
    """
    n_items = len(breakdown.names)
    expected = (*leading, n_items)
    hint = "(n_windows, n_items)" if leading else "(n_items,)"
    # Only the single-point message points back at `names`.
    suffix = "" if leading else f" to match `{name}.names`"

    fields_to_check = list(_BREAKDOWN_FIELDS)
    if name == "assets":
        fields_to_check.extend(_ASSET_BREAKDOWN_FIELDS)
    ...
    for field in fields_to_check:
        ...
        label = f"{name}.{field}"
        _check_ndim(label, value, expected, hint)
        _check_shape(label, value, expected, suffix=suffix)

    for field, scalar_allowed in _OPTIONAL_BREAKDOWN_FIELDS:
        value = getattr(breakdown, field, None)
        if value is None or (scalar_allowed and not isinstance(value, np.ndarray)):
            continue
        _check_shape(f"{name}.{field}", value, expected)
...
def _prepare_plot_data(
    ...
    include_idio: bool,
    is_rolling: bool,
) -> tuple[list[str], dict[str, FloatArray], list[str]]:
    """Prepare ordered component data for attribution plots.

    Selects the top components, aggregates the remainder into `Other` and optionally
    appends the idiosyncratic component.
    ...
    display_names = [format_plot_label(name) for name in names]

    return display_names, values, colors


def _prepare_exposure_plot_data(
    data: FactorBreakdown | FamilyBreakdown,
    top_n: int | None,
    is_rolling: bool,
) -> tuple[list[str], FloatArray, FloatArray | None, list[str]]:
    """Prepare ordered exposure data for exposure plots."""
    _validate_no_reserved_component_names(data.names.tolist())

    exposure = _as_2d(data.exposure)
    ...
    if data.exposure_std is not None:
        exposure_std_values = _as_2d(data.exposure_std)[:, top_idx]

    if other_idx is not None:
        names.append("Other")
        other_exposure = exposure[:, other_idx].sum(axis=1, keepdims=True)
        exposure_values = np.hstack([exposure_values, other_exposure])

        if exposure_std_values is not None:
            other_std = np.full((exposure_std_values.shape[0], 1), np.nan)
            exposure_std_values = np.hstack([exposure_std_values, other_std])

    color_map = _make_component_color_map(data.names.tolist())
    colors = _colors_for_names(names, color_map)
    display_names = [format_plot_label(name) for name in names]

    return display_names, exposure_values, exposure_std_values, colors
...
def _prepare_perf_mu_uncertainty_se(
    ...
    ndarray of shape (n_windows, n_items) or None
        Standard errors aligned with plotted components. `None` when the breakdown has
        no per-item uncertainty.
    """
...
def _perf_contrib_hover_html(
    ...
    When a confidence level and finite standard error are available, the mean return
    line matches formatted DataFrames:
...
def _color_with_alpha(color: str, alpha: float) -> str:
    """Return `color` with an alpha channel for Plotly fills."""
...
def _add_scatter_band(
    ...
    """Add a translucent band centered on `y` with half-width `width`."""
    valid = np.isfinite(y) & np.isfinite(width) & (width >= 0)
    if not np.any(valid):
        return

    lower = np.where(valid, y - width, np.nan)
    upper = np.where(valid, y + width, np.nan)
    fig.add_trace(
        go.Scatter(
            x=x,
            y=lower,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            line=dict(width=0),
            fill="tonexty",
            fillcolor=_color_with_alpha(color, 0.16),
            hoverinfo="skip",
            showlegend=False,
            name=f"{name} {label}",
            legendgroup=legendgroup,
...
def _plot_rolling_lines(
    ...
    hovertemplates: list[str] | None = None,
    hovertexts: list[list[str]] | None = None,
    customdata: list[AnyArray] | None = None,
    band_width: FloatArray | None = None,
    band_label: str = "Band",
) -> go.Figure:
    """Plot rolling components as line traces with optional symmetric bands."""
    fig = go.Figure()
    for i, name in enumerate(names):
        legendgroup = f"{i}:{name}"
        ...
        if customdata is not None:
            trace_kwargs["customdata"] = customdata[i]
        if hovertexts is not None:
            trace_kwargs["hovertext"] = hovertexts[i]
            trace_kwargs["hoverinfo"] = "text"
        elif hovertemplates is not None:
            trace_kwargs["hovertemplate"] = hovertemplates[i]
...
def _plot_contribution_chart(
    ...
    if is_risk:
        ...
        yaxis_title = "Annualized Volatility Contribution (%)"
    ...
    if is_rolling:
        if observations is None:
            raise ValueError("observations required for rolling contribution chart")
        ...
        if show_unc_band:
            ...
            ci_pct = round(confidence_level * 100)
            band_label = f"{ci_pct}% CI"
            layout_title = {
                "text": (
                    f"{title_rolling}<br><sup style='font-size:0.7em'>"
                    f"Band: {ci_pct}% CI for mean return contribution</sup>"
                )
            }
