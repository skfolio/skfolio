...
class BaseBreakdown(ABC):
    ...
    def _build_df(
        ...
        if self._is_rolling:
            ...
            df = df.iloc[np.argsort(-np.abs(self.pct_total_variance))].set_index(
                names_col
            )
...
class AssetBreakdown(BaseBreakdown):
    ...
    def _to_df(
        ...
        -------
        df : pandas.DataFrame
            Pandas DataFrame with asset-level attribution data. For single-point
            attribution, indexed by `Asset`.
        """
        mu_label = "Mean Return" if is_realized else "Expected Return"
