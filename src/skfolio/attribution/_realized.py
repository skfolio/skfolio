...
def realized_factor_attribution(
    ...
    **Time convention (as-of indexing):**
    ...
    information available up to and including the end of period :math:`t`.
    Point-in-time fields and derived values store the latest available value for
    observation :math:`t`. Returns stored at observation :math:`t` cover the period
    ending at :math:`t`, namely :math:`(t-1, t]`.

    For time-varying exposures, attribution uses exposures from before the return
    interval. When `exposure_lag > 0`, the function aligns :math:`B_{t-\ell}` with
    returns at :math:`t`; the first :math:`\ell` return observations are discarded.
    ...
    **Return Attribution:**
    ...
    exposure_lag : int, default=1
        ...
        :math:`(t-1, t]`. Only affects 3D (time-varying) exposures.
    ...
    **NaN handling:**
    ...
    >>> portfolio_returns = (
    ...     factor_returns @ (loading_matrix.T @ weights) + residuals @ weights
    ... )
    ...
    Exposure std (shows position dynamism): [0.0122634  0.00836653 0.01058102]
    """
    ...
    # Under the as-of time-indexing convention, exposures[t] = B_t and returns[t] = R_t.
...
def rolling_realized_factor_attribution(
    *,
    ...
    Total vol over time: [0.26478682 0.28232785 0.28856211 0.27556505 0.2311759  0.23023946
...
def _prepare_uncertainty_inputs(
    regression_weights: FloatArray,
    idio_variances: FloatArray,
    inactive_mask: BoolArray,
) -> tuple[FloatArray, FloatArray]:
    """Zero unavailable asset-observation pairs in uncertainty inputs."""
    unavailable_mask = inactive_mask | np.isnan(idio_variances)
    regression_weights = np.where(unavailable_mask, 0.0, regression_weights)
    idio_variances = np.where(unavailable_mask, 0.0, idio_variances)
...
def _compute_attribution_uncertainty(
    ...
    n_factors = full_exposures.shape[-1]
    if factor_families is None:
        factor_is_currency = np.zeros(n_factors, dtype=bool)
    else:
        factor_is_currency = factor_families == _CURRENCY
    ...
    if family_constraint_basis is not None:
        if exposure_is_static:
            full_exposures_for_basis = np.broadcast_to(
                full_exposures[np.newaxis],
                (n_observations, n_assets, full_exposures.shape[1]),
            )
        else:
            full_exposures_for_basis = full_exposures
        regression_exposures = family_constraint_basis.reduce_exposures(
            full_exposures_for_basis
        )
        regression_portfolio_exposure = (
            family_constraint_basis.project_factor_coordinates(full_portfolio_exposure)
        )
        reduced_factor_families = (
            family_constraint_basis.reduced_factor_names(factor_families)
            if factor_families is not None
            else None
        )
        reduced_factor_is_currency = (
            reduced_factor_families == _CURRENCY
            if reduced_factor_families is not None
            else np.zeros(regression_exposures.shape[2], dtype=bool)
        )
        regression_factor_mask = ~reduced_factor_is_currency
    else:
        regression_exposures = full_exposures
        regression_portfolio_exposure = full_portfolio_exposure
        regression_factor_mask = ~factor_is_currency

    if not np.all(regression_factor_mask):
        if regression_exposures.ndim == 2:
            regression_exposures = regression_exposures[:, regression_factor_mask]
        else:
            regression_exposures = regression_exposures[:, :, regression_factor_mask]
        regression_portfolio_exposure = regression_portfolio_exposure[
            :, regression_factor_mask
        ]
    ...
    # Middle term
    sandwich_weights = reg_weights * reg_weights * idio_var
    if regression_exposures.ndim == 2:
        sandwich_middle_term = np.einsum(
            "tn,nk,nl->tkl",
            sandwich_weights,
            regression_exposures,
            regression_exposures,
    ...
    # Map covariance back to full basis for per-factor and per-family SEs. Currency
    # factors are direct factor returns, so their regression-estimation uncertainty is
    # reported as NaN.
    if family_constraint_basis is not None:
        n_reduced = family_constraint_basis.n_reduced_factors
        factor_covariance_reduced = np.zeros(
            (n_observations, n_reduced, n_reduced), dtype=float
        )
        regression_idx = np.flatnonzero(regression_factor_mask)
        factor_covariance_reduced[:, regression_idx[:, np.newaxis], regression_idx] = (
            factor_covariance_regression
        )
        factor_covariance = family_constraint_basis.expand_factor_covariance(
            factor_covariance_reduced
        )
    else:
        factor_covariance = np.zeros(
            (n_observations, n_factors, n_factors), dtype=float
        )
        regression_idx = np.flatnonzero(regression_factor_mask)
        factor_covariance[:, regression_idx[:, np.newaxis], regression_idx] = (
            factor_covariance_regression
        )
    ...
    per_factor_uncertainty[factor_is_currency] = np.nan
