...
from skfolio.utils.tools import check_estimator, half_life_to_decay_factor
...
class PredictorAlpha(BaseAlphaDescriptorComposition, BaseAlpha):
    r"""Predictor alpha estimator using a user-provided regressor.
     ...
     idiosyncratic return units when rescaling is enabled.

     The predictor supports two target models. With `scale_target_by_idio_vol=False`, it
     is fitted to the forward mean idiosyncratic return :math:`u_{t,i}`. With
     `scale_target_by_idio_vol=True`, it is fitted to :math:`u_{t,i} / \sigma_{t,i}` and
     the forecast is multiplied by current idiosyncratic volatility so `alpha_` remains
     in expected return units. This is useful when signals are assumed to forecast
     idiosyncratic Sharpe rather than raw idiosyncratic return. For the same scaled
     signal forecast, higher-volatility assets receive larger return alpha because the
     forecast is converted back from idiosyncratic-Sharpe units to return units.

     When `rescale_to_expected_returns=True`, the raw predictor output
    ...
    ----------
     ...
     half_life : float, default=20
         Half-life of the EWLS rescaling statistics in number of observations. Only used
         when `rescale_to_expected_returns=True`.
     ...
     target_scoring_transformer : BaseCSTransformer or "passthrough", optional
         ...
         `"passthrough"`. The rescaling stage calibrates the predictor output back to
         expected-return units when `rescale_to_expected_returns=True`.
     ...
     scale_target_by_idio_vol : bool, default=False
         If `False`, the predictor is trained on the forward mean idiosyncratic
         return. If `True`, the target is divided by forecast idiosyncratic volatility
         and the resulting idiosyncratic-sharpe forecast is converted back to return
         units by multiplying by current idiosyncratic volatility.

     rescale_to_expected_returns : bool, default=True
         ...
         conversion implied by `scale_target_by_idio_vol`.

     cv : cross-validator, int, or None, default=None
         Cross-validation strategy used to obtain predictions from held-out folds for
         rescaling calibration. When `None`, uses `KFold(5)`. CV is used only in
         batch mode when `rescale_to_expected_returns=True` and there are enough
         samples.
    ...
    ----------
     alpha_ : ndarray of shape (n_assets,) or None
         Estimated alpha for each asset. If `rescale_to_expected_returns=True`, this
         is in expected return units. Otherwise, it is the predictor output after any
         volatility conversion. Returns `None` during warmup.
    ...
    --------
     ...
     >>> from skfolio.factor_model.alpha import PredictorAlpha
    ...
    def __init__(
        ...
        *,
        ...
        rescale_to_expected_returns: bool = True,
        half_life: float = 20,
        ...
        self.rescale_to_expected_returns = rescale_to_expected_returns
    ...
    def fit(self, X: AssetPanel, y=None, **fit_params) -> PredictorAlpha:
        ...
        internal state and fits on the provided data. When rescaling is enabled,
        ...
        X : AssetPanel
            ...
            "idio_variances" for rescaling or volatility-scaled targets, and
    ...
    def _fit(
        ...
        **fit_params,
        ...
        if self.rescale_to_expected_returns or self.scale_target_by_idio_vol:
        ...
        if self.rescale_to_expected_returns:
            # Rescaling uses pre-update predictions when possible. This avoids
            ...
            uncalibrated_alpha_for_rescaling = (
        ...
        if self.rescale_to_expected_returns:
            if uncalibrated_alpha_for_rescaling is None:
                # The first incremental update has no previous predictor and no
                # CV path, so rescaling starts from newly fitted predictions.
                uncalibrated_alpha_for_rescaling = self._predict_uncalibrated_alpha(
                    scores=X[_DESCRIPTOR_SCORES][train_slice],
                    idio_variances=X[_IDIO_VARIANCES][train_slice],
                )

            self._update_rescaling(
                uncalibrated_alpha=uncalibrated_alpha_for_rescaling,
        ...
        # available rescaling coefficient.
        uncalibrated_alpha = self._predict_uncalibrated_alpha(
            scores=latest_scores, idio_variances=latest_idio_variances
        )
        self._set_alpha(uncalibrated_alpha)
        ...
        *,
        ...
        if method == "fit":
            ...
            if n_valid_samples >= n_splits * _MIN_SAMPLES_PER_FOLD:
                ...
                if not self.scale_target_by_idio_vol:
                    return alpha
    ...
    def _predict_uncalibrated_alpha(
        ...
        """Predict alpha before applying the rescaling coefficient."""
        ...
        if idio_variances is None:
            raise ValueError(
                "idio_variances are required when scale_target_by_idio_vol=True"
            )
        return alpha * np.sqrt(idio_variances)

    def _update_rescaling(
        self,
        *,
        ...
        for alpha_t, target_t, weights_t, valid_t in zip(
            ...
            self._rescaling_normal *= self._rescaling_decay
            self._rescaling_normal += (1.0 - self._rescaling_decay) * obs_normal
            self._rescaling_cross *= self._rescaling_decay
            self._rescaling_cross += (1.0 - self._rescaling_decay) * obs_cross
            self._n_valid_rescaling_obs += 1
            ridge = _RESCALING_RIDGE_SCALE * max(
                abs(self._rescaling_normal), np.finfo(float).eps
            )
            self.rescaling_coef_ = self._rescaling_cross / (
                self._rescaling_normal + ridge
            )

    def _set_alpha(self, uncalibrated_alpha: FloatArray) -> None:
        """Set `alpha_` from an uncalibrated alpha prediction."""
        if not self.rescale_to_expected_returns:
            self.alpha_ = uncalibrated_alpha
        elif np.isfinite(self.rescaling_coef_):
            self.alpha_ = self.rescaling_coef_ * uncalibrated_alpha
        else:
            self.alpha_ = None
    ...
    def _validate_params(self) -> None:
        ...
        if not isinstance(self.rescale_to_expected_returns, (bool, np.bool_)):
            raise ValueError(
                "rescale_to_expected_returns must be a boolean, "
                f"got {self.rescale_to_expected_returns!r}"
            )
    ...
    def _initialize(self) -> None:
        ...
        if self.rescale_to_expected_returns:
            self._rescaling_decay = half_life_to_decay_factor(self.half_life)
            self._rescaling_normal = 0.0
            self._rescaling_cross = 0.0
            self._n_valid_rescaling_obs = 0
            self.rescaling_coef_ = np.nan

    @property
    def _needs_idio_variances(self) -> bool:
        return self.rescale_to_expected_returns or self.scale_target_by_idio_vol
