...
vine = VineCopula(log_transform=True, central_assets=["BAC"], n_jobs=-1, random_state=0)
vine.fit(X)
vine.display_vine()
...
X_stressed = vine.sample(n_samples=50_000, conditioning=conditioning)

# %%
# Let's plot the marginal distribution of the stressed assets and compare them with the
# historical data:
vine.plot_marginal_distributions(X, conditioning=conditioning)
