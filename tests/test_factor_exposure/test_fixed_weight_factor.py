...
@pytest.fixture
def characteristics_factor_model_inputs(make_characteristics_panel):
    characteristics = make_characteristics_panel(
        n_assets=80,
        n_observations=252,
        random_state=42,
...
def test_non_finite_descriptor_values_are_excluded(simple_panel):
    """Test that NaN and infinite descriptor values are excluded from composites."""
    simple_panel["market_cap"][0, 0] = np.inf
    simple_panel["book_equity"][1, 0] = -np.inf
    simple_panel["market_cap"][2, 0] = np.nan
    simple_panel["book_equity"][2, 0] = np.inf
    ...
    np.testing.assert_allclose(result[0, 0], simple_panel["book_equity"][0, 0])
    np.testing.assert_allclose(result[1, 0], simple_panel["market_cap"][1, 0])
...
def test_characteristics_factor_model_fit_routes_descriptor_metadata(
    characteristics_factor_model_inputs,
):
    X, characteristics, reference_returns = characteristics_factor_model_inputs
