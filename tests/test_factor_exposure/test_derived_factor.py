...
class TestCharacteristicsFactorModelWithDerived:
    ...
    def test_fit_with_derived_factor(self, dense_characteristics_panel):
        """Test full model fit with a DerivedFactor."""
        characteristics = dense_characteristics_panel
    ...
    def test_fit_with_neutralization(self, dense_characteristics_panel):
        ...
        model = CharacteristicsFactorModel(factors=factors)
        neutralized_model = CharacteristicsFactorModel(
            factors=factors, neutralize_against={"nlsize": ["size"]}
        )

        model.fit(X, characteristics=characteristics)
        neutralized_model.fit(X, characteristics=characteristics)

        def _size_nlsize_corr(model):
