...
class _EstimatorWithInternalParam:
    def fit(self, X, y=None, return_distribution=None):
        pass
...
class TestInputToArray:
    ...
    @pytest.mark.parametrize(
        ("overrides", "match"),
        [
            ({"dim": 3}, "dim must be 1 or 2"),
            ({"items": [[1, 2]]}, "must be a 1D array"),
            ({"items": [1, np.nan]}, "contains NaN"),
            ({"items": [1], "n_assets": 2}, "shape"),
            ({"items": [[1, 2]], "n_assets": 3, "dim": 2}, "shape"),
        ],
    )
    def test_rejects_invalid_array_inputs(self, overrides, match):
        params = {
            "items": [1, 2],
            "n_assets": 2,
            "fill_value": 0,
        ...
        with pytest.raises(ValueError, match=match):
            input_to_array(**params)

    def test_convert_group_dictionary_to_2d_array(self):
        result = input_to_array(
            {"A": "group", "B": ["style", "risk"]},
            n_assets=2,
            fill_value="",
            dim=2,
...
def test_validate_input_list_warns_for_missing_name_when_requested():
    with pytest.warns(UserWarning, match="MISSING not found"):
        result = validate_input_list(
            ["MISSING"],
            n_assets=2,
            assets_names=np.array(["A", "B"]),
...
class TestCallEstimator:
    ...
    def test_raises_on_missing_partial_fit_transform(self):
        estimator = _RecordingEstimator()

        with pytest.raises(TypeError, match="does not implement partial_fit_transform"):
            _call_estimator(
                estimator,
...
def test_filter_supported_params():
    return_distribution = object()
    filtered = _filter_supported_params(
        _EstimatorWithInternalParam(),
        "fit",
