...
def _ffill_market_cap(panel):
    """Forward-fill market_cap NaN holes so finite_in_universe validation passes."""
    mc = panel["market_cap"]
    for j in range(mc.shape[1]):
        for i in range(1, mc.shape[0]):
            if np.isnan(mc[i, j]):
                mc[i, j] = mc[i - 1, j]
    return panel
...
@pytest.fixture
def panel_data():
    """Synthetic panel with NaN (delistings, late listings)."""
    panel = generate_synthetic_characteristics(
        n_assets=20,
        years=0.5,
        seed=42,
        mid_nan_frac=0.01,
    ...
    return _ffill_market_cap(panel)


@pytest.fixture
def clean_panel_data():
    """Synthetic panel with no NaN for deterministic testing."""
    return generate_synthetic_characteristics(
        n_assets=10,
        years=0.3,
        seed=123,
        mid_nan_frac=0.0,
...
class TestParameterValidation:
    ...
    def test_invalid_aggregation_period(self, clean_panel_data, ref_returns):
        ...
        with pytest.raises(ValueError, match="aggregation_period must be >= 1"):
            ms.fit_transform(clean_panel_data, reference_returns=ref_returns)
