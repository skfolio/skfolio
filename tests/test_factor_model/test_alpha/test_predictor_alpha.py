...
class TestWarmupBehavior:
    ...
    def test_warmup_then_fit_with_batch_only_predictor(self, alpha_deterministic_panel):
        ...
        model = PredictorAlpha(
            ...
            horizon=5,
            half_life=5,
            rescale_to_expected_returns=False,
        )
...
class TestRescaling:
    """Tests for rescale_to_expected_returns parameter."""

    def test_no_rescale(self, alpha_deterministic_panel):
        """Test with rescale_to_expected_returns=False."""
        model = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            horizon=3,
        ...
        assert model.alpha_ is not None
        # Without rescaling, alpha is raw predictor output

    def test_rescale_vs_no_rescale_differ(self, alpha_deterministic_panel):
        """Test that rescaled and non-rescaled alpha differ."""
        model_rescale = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            horizon=3,
            rescale_to_expected_returns=True,
        )
        model_rescale.fit(alpha_deterministic_panel)

        model_no_rescale = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            horizon=3,
            rescale_to_expected_returns=False,
        )
        model_no_rescale.fit(alpha_deterministic_panel)

        # Results should differ
        assert not np.allclose(model_rescale.alpha_, model_no_rescale.alpha_)

    def test_scale_target_by_idio_vol(self, alpha_deterministic_panel):
        """Volatility-scaled target mode should still return a finite alpha."""
        model = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            horizon=3,
    ...
    def test_rescaling_uses_ewls_statistics_not_smoothed_coefficients(self):
        ...
        model._rescaling_decay = 0.5
        model._rescaling_normal = 0.0
        model._rescaling_cross = 0.0
        model._n_valid_rescaling_obs = 0
        model.rescaling_coef_ = np.nan

        model._update_rescaling(
        ...
        np.testing.assert_allclose(model.rescaling_coef_, expected_ewls_coefficient)
        assert not np.allclose(model.rescaling_coef_, smoothed_daily_coefficient)

    def test_estimation_mask_excludes_rescaling_observations(self):
        ...
        np.testing.assert_allclose(model.rescaling_coef_, 1.0 / (1.0 + 1.0e-6))
...
class TestTargetTransformers:
    ...
    def test_target_transformer_receives_weights_and_groups_without_rescale(
        self, alpha_deterministic_panel
    ):
        """Target transforms should keep estimation weights when rescaling is disabled."""
        panel = alpha_deterministic_panel.copy(deep=True)
...
class TestValidation:
    ...
    def test_non_bool_scale_target_by_idio_vol_raises(self, alpha_deterministic_panel):
        """Non-boolean scale_target_by_idio_vol should raise ValueError."""
        model = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            scale_target_by_idio_vol="bad",
        )
        with pytest.raises(
            ValueError, match="scale_target_by_idio_vol must be a boolean"
        ):
            model.fit(alpha_deterministic_panel)

    def test_non_bool_rescale_to_expected_returns_raises(
        self, alpha_deterministic_panel
    ):
        """Non-boolean rescale_to_expected_returns should raise ValueError."""
        model = PredictorAlpha(
            predictor=Ridge(alpha=1.0),
            descriptors=[("signal", Passthrough("signal"))],
            rescale_to_expected_returns="bad",
        )
        with pytest.raises(
            ValueError, match="rescale_to_expected_returns must be a boolean"
        ):
            model.fit(alpha_deterministic_panel)
...
class TestRegression:
    ...
    def test_exact_coef_values(self, alpha_deterministic_panel):
        ...
        assert np.isfinite(model.rescaling_coef_)
        assert model._n_valid_rescaling_obs > 0

        # Expected coefficient (computed with current implementation)
        # To regenerate: print(repr(model.rescaling_coef_))
        expected_coef = 0.88278537

        np.testing.assert_allclose(
            model.rescaling_coef_,
            expected_coef,
            rtol=1e-5,
