...
class TestAttributionPlotMethods:
    ...
    def test_plot_exposure_contains_factors(self, simple_factor_model):
        ...
        result = predicted_factor_attribution(**simple_factor_model)
        bar_data = result.plot_exposure(top_n=None).data[0]
        assert "Momentum" in bar_data.x
        assert "Value" in bar_data.x
        assert "Idiosyncratic" not in bar_data.x

    def test_plot_exposure_title(self, simple_factor_model):
        """Test that exposure chart has correct title."""
        result = predicted_factor_attribution(**simple_factor_model)
        assert "Factor Exposure" in result.plot_exposure().layout.title.text

    def test_plot_exposure_by_family(self, model_with_families):
        """Test that exposure chart can aggregate by factor family."""
        result = predicted_factor_attribution(**model_with_families)
        bar_data = result.plot_exposure(by_family=True, top_n=None).data[0]
        assert "Style" in bar_data.x
        assert "Value" in bar_data.x

    def test_plot_exposure_realized_shows_std(self, static_realized_model):
        """Test that realized exposure chart displays exposure std."""
        result = realized_factor_attribution(**static_realized_model)
        fig = result.plot_exposure(show_std=True)
        bar_data = fig.data[0]
        assert bar_data.error_y.visible is True
        assert len(bar_data.error_y.array) == result.n_factors
        assert "Error bars: +/- 1 exposure std" in fig.layout.title.text

    def test_plot_exposure_rolling_line_chart(self, rolling_time_varying_model):
        """Test that rolling exposure chart returns lines and std bands."""
        result = rolling_realized_factor_attribution(
            **rolling_time_varying_model, window_size=60, step=20
        )
        fig = result.plot_exposure(top_n=2)

        assert isinstance(fig, go.Figure)
        assert all(trace.type == "scatter" for trace in fig.data)
        assert all(len(trace.x) == len(result.observations) for trace in fig.data)
        assert any(trace.fill == "tonexty" for trace in fig.data)
        assert all(getattr(trace.error_y, "array", None) is None for trace in fig.data)
        band = next(trace for trace in fig.data if trace.fill == "tonexty")
        line = next(
            trace
            for trace in fig.data
            if trace.legendgroup == band.legendgroup and trace.showlegend is not False
        )
        assert line.fill is None
        trace_names = [trace.name for trace in fig.data]
        assert "Other" in trace_names
        assert "Idiosyncratic" not in trace_names
    ...
    def test_plot_labels_formatted(self, simple_factor_model):
        model = {
            **simple_factor_model,
            "factor_names": ["non_linear_size", "earnings_yield"],
        }
        result = predicted_factor_attribution(**model)
        bar = result.plot_vol_contrib(top_n=None, include_idio=False).data[0]
        assert list(bar.x) == ["Non Linear Size", "Earnings Yield"]
    ...
    def test_single_factor_plot(self):
        ...
        assert "Single Factor" in bar_data.x
        assert "Idiosyncratic" in bar_data.x
    ...
    def test_plot_vol_contrib_rolling_line_chart(self, rolling_static_model):
        """Test that rolling plot_vol_contrib returns one line per component."""
        result = rolling_realized_factor_attribution(
...
class TestAssetAttribution:
    ...
    def test_assets_df_structure(self, simple_factor_model):
        ...
        assert assets_df.index.name == "Asset"
    ...
    def test_asset_vol_contrib_sums_to_total(self, simple_factor_model):
        ...
        result = predicted_factor_attribution(
            **simple_factor_model, annualization_factor=1
        )
    ...
    @pytest.mark.parametrize(
        "formatted,expected_dtype", [(True, "string"), (False, np.float64)]
    )
    def test_assets_df_formatting(self, simple_factor_model, formatted, expected_dtype):
        ...
        dtype = assets_df["Standalone Volatility"].dtype
