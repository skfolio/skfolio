...
class TestAttributionUncertaintyPresence:
    ...
    def test_uncertainty_absent_when_inputs_missing(self, static_uncertainty_model):
        ...
        result = realized_factor_attribution(**model, annualization_factor=1)
...
class TestAttributionUncertaintyMath:
    ...
    def test_se_scales_with_annualization(self, static_uncertainty_model):
        """SE should scale linearly with annualization_factor."""
        result_raw = realized_factor_attribution(
            **static_uncertainty_model, annualization_factor=1
        )
        result_ann = realized_factor_attribution(
            **static_uncertainty_model, annualization_factor=252
        )
    ...
    def test_se_decreases_with_more_obs(self):
        ...
        result_short = realized_factor_attribution(
            **model_short, annualization_factor=1
        )
        result_long = realized_factor_attribution(**model_long, annualization_factor=1)
    ...
    def test_gls_case_agrees_with_generic(self, gls_uncertainty_model):
        ...
        result = realized_factor_attribution(
            **gls_uncertainty_model, annualization_factor=1
        )
        assert result.systematic.mu_uncertainty > 0
    ...
    def test_time_varying_exposures(self, time_varying_uncertainty_model):
        ...
        result = realized_factor_attribution(
            **time_varying_uncertainty_model, annualization_factor=1
        )
        assert result.systematic.mu_uncertainty is not None
        assert result.systematic.mu_uncertainty > 0
        assert result.idio.mu_uncertainty > 0

    def test_idio_variance_warmup_is_ignored(self, static_uncertainty_model):
        """Missing idio variance entries are excluded from uncertainty inputs."""
        model = static_uncertainty_model.copy()
        model["idio_variances"] = model["idio_variances"].copy()
        model["idio_variances"][:5] = np.nan

        result = realized_factor_attribution(**model, annualization_factor=1)

        expected_model = static_uncertainty_model.copy()
        expected_model["regression_weights"] = expected_model[
            "regression_weights"
        ].copy()
        expected_model["idio_variances"] = expected_model["idio_variances"].copy()
        expected_model["regression_weights"][:5] = 0.0
        expected_model["idio_variances"][:5] = 0.0
        expected = realized_factor_attribution(**expected_model, annualization_factor=1)

        np.testing.assert_almost_equal(result.total.vol, expected.total.vol)
        np.testing.assert_almost_equal(
            result.systematic.mu_uncertainty,
            expected.systematic.mu_uncertainty,
        )

    def test_internal_idio_variance_nan_is_ignored(self, static_uncertainty_model):
        """Internal missing idio variance entries are excluded from uncertainty."""
        model = static_uncertainty_model.copy()
        model["idio_variances"] = model["idio_variances"].copy()
        ...
        expected_model["idio_variances"] = expected_model["idio_variances"].copy()
        expected_model["regression_weights"][10, 0] = 0.0
        expected_model["idio_variances"][10, 0] = 0.0
        expected = realized_factor_attribution(**expected_model, annualization_factor=1)
    ...
    def test_unused_idio_variance_nan_is_ignored(self, static_uncertainty_model):
        """Assets with zero regression weight do not require idio variance."""
        model = static_uncertainty_model.copy()
        model["idio_variances"] = model["idio_variances"].copy()
        model["regression_weights"] = model["regression_weights"].copy()
        model["idio_variances"][10, 0] = np.nan
        model["regression_weights"][10, 0] = 0.0
...
class TestAttributionUncertaintyDataFrame:
    ...
    def test_summary_df_has_uncertainty(self, static_uncertainty_model):
        ...
        systematic_se = df.loc["Systematic", "Mean Return Uncertainty"]
        idio_se = df.loc["Idiosyncratic", "Mean Return Uncertainty"]
        total_se = df.loc["Total", "Mean Return Uncertainty"]
...
class TestAttributionUncertaintyRolling:
    ...
    def test_rolling_idio_variance_warmup_is_ignored(self, rolling_uncertainty_model):
        model = rolling_uncertainty_model.copy()
        model["idio_variances"] = model["idio_variances"].copy()
        model["idio_variances"][:5] = np.nan
        ...
        expected_labels = rolling_uncertainty_model["observations"][
            np.arange(0, 200 - 60 + 1, 20) + 59
        ]
        np.testing.assert_array_equal(result.observations, expected_labels)
...
class TestAttributionUncertaintyValidation:
    ...
    def test_compute_uncertainty_true_missing_both(self, static_uncertainty_model):
        ...
        with pytest.raises(ValueError, match="compute_uncertainty=True"):
            realized_factor_attribution(
                **model, annualization_factor=1, compute_uncertainty=True
            )
    ...
    def test_compute_uncertainty_false_ignores_partial_inputs(
        ...
        result = realized_factor_attribution(
            **model, annualization_factor=1, compute_uncertainty=False
        )
        assert result.systematic.mu_uncertainty is None
    ...
    def test_error_nan_in_regression_weights(self, static_uncertainty_model):
        ...
        with pytest.raises(ValueError, match="`regression_weights` contains"):
            realized_factor_attribution(**model, annualization_factor=1)
...
class TestAttributionUncertaintyNaN:
    ...
    def test_nan_idio_variances_active_cell_is_ignored(self):
        """NaN in idio_variances excludes that asset from uncertainty."""
        model = _create_uncertainty_model(n_obs=50, n_assets=4, n_factors=2, seed=77)
        model["idio_variances"][10, 0] = np.nan
    ...
    def test_nan_idio_variances_equivalent_to_zero(self):
        ...
        r_nan = realized_factor_attribution(**model_nan, annualization_factor=1)
        r_zero = realized_factor_attribution(**model_zero, annualization_factor=1)
    ...
    def test_rolling_nan_idio_variances(self):
        ...
        result = rolling_realized_factor_attribution(
            **model, window_size=30, step=20, annualization_factor=1
        )
...
class TestPlotReturnContribUncertainty:
    """Return chart uncertainty: single-point bars and rolling bands."""
    ...
    def test_single_point_error_y_with_confidence_level(self, static_uncertainty_model):
        ...
        assert (
            "Error bars: 95% CI for mean return contribution" in fig.layout.title.text
        )
        ht = bar.hovertext[0]
        assert "(95% CI):" in ht
        assert " ± " in ht
    ...
    def test_rolling_line_band_hover_shows_ci(self, rolling_uncertainty_model):
        result = rolling_realized_factor_attribution(
            **rolling_uncertainty_model,
            window_size=60,
            step=30,
        ...
        assert any(tr.fill == "tonexty" for tr in fig.data)
        for tr in fig.data:
            assert not _error_y_has_visible_interval(tr)
        band = next(tr for tr in fig.data if tr.fill == "tonexty")
        grouped_line = next(
            tr
            for tr in fig.data
            if tr.legendgroup == band.legendgroup and tr.showlegend is not False
        )
        assert grouped_line.fill is None

        line = next(tr for tr in fig.data if getattr(tr, "hovertext", None) is not None)
        ht = line.hovertext[0]
        assert "(95% CI):" in ht
        assert " ± " in ht
        assert "Mean return SE" not in ht
    ...
    def test_rolling_line_no_band_when_confidence_level_none(
        self, rolling_uncertainty_model
    ):
        result = rolling_realized_factor_attribution(
            **rolling_uncertainty_model,
            window_size=60,
        ...
        fig = result.plot_return_contrib(confidence_level=None)
        assert not any(tr.fill == "tonexty" for tr in fig.data)
    ...
    def test_without_uncertainty_data_legacy_hover(self, static_realized_model):
        result = realized_factor_attribution(
            **static_realized_model, annualization_factor=1
        )
        fig = result.plot_return_contrib(confidence_level=0.95)
        bar = fig.data[0]
        assert not _error_y_has_visible_interval(bar)
