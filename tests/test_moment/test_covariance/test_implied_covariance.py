...
def test_compute_realised_vol(X):
    ...
    while (k + 1) * window <= n_observations:
        res.append(
            np.asarray(
                X.iloc[n_observations - (k + 1) * window:n_observations - k * window]
                .std(axis=0, ddof=0)
            )
        )
        k += 1
...
def test_compute_implied_vol(implied_vol):
    ...
    while (k + 1) * window <= n_observations:
        res.append(
            np.asarray(
                implied_vol.iloc[n_observations - k * window - 1]
            )
        )
        k += 1
    res = np.flip(np.array(res), 0)
...
def test_implied_covariance_default(X, implied_vol):
    ...
    np.testing.assert_almost_equal(model.covariance_[:2][:2], np.array(
        [[6.57488641e-04, 2.83134035e-04, 1.56184082e-04, 1.75814250e-04,
          1.22056078e-04, 1.38660138e-04, 1.47083022e-04, 8.10806087e-05,
          1.42473991e-04, 8.01598741e-05, 9.68377637e-05, 7.98446492e-05,
          2.15007353e-04, 9.97345580e-05, 8.60888400e-05, 8.56609339e-05,
          1.37569074e-04, 1.32999989e-04, 8.21417385e-05, 1.06253398e-04],
         [2.83134035e-04, 1.01675318e-03, 2.20576378e-04, 2.51503645e-04,
          1.76240627e-04, 1.87494904e-04, 2.00154514e-04, 8.31528862e-05,
          1.81429439e-04, 8.63395567e-05, 1.16628151e-04, 9.03413258e-05,
          2.57876321e-04, 1.02332010e-04, 1.10774884e-04, 8.74268879e-05,
          2.68487267e-04, 1.62195796e-04, 8.62525325e-05, 1.38431233e-04]]))

    np.testing.assert_almost_equal(np.diag(model.covariance_),
                                   model.pred_realised_vols_ ** 2)
    np.testing.assert_almost_equal(model.pred_realised_vols_, np.array(
        [0.02564154, 0.03188657, 0.02002452, 0.02736693, 0.01973539,
         0.02245573, 0.01815133, 0.00957966, 0.01799403, 0.01123385,
         0.01780456, 0.01307078, 0.02186195, 0.01093394, 0.0151937,
         0.01116985, 0.03529737, 0.01416595, 0.0128378, 0.01956481]))
...
@pytest.mark.parametrize("volatility_risk_premium_adj",
                         [0.1, 1, 1.5, 100])
def test_implied_covariance_volatility_risk_premium_adj(X, implied_vol,
...
@pytest.mark.parametrize("volatility_risk_premium_adj",
                         [-0.5, 0])
def test_implied_covariance_volatility_risk_premium_adj_non_pos(X, implied_vol,
                                                                volatility_risk_premium_adj):
    model = ImpliedCovariance(volatility_risk_premium_adj=volatility_risk_premium_adj)
    with pytest.raises(ValueError):
        model.fit(X, implied_vol=implied_vol)
...
@pytest.mark.parametrize("n_folds",
                         [0.1, 1, 2])
